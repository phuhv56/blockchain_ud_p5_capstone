...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        emit Transferred(_owner, newOwner);
        _owner = newOwner;
    }
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(!_paused, 'Is Unpaused');
        _;
    }
    modifier paused() {
        require(_paused, 'Is Pause');
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender != currentOwner, 'the msg sender to be the owner of the contract or isApprovedForAll() to be true');
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId), 'revert if given tokenId already exists or given address is invalid');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == currentOwner, 'from address is the owner of the given token');

        // TODO: require token is being transfered to valid address
        require(to != currentOwner, 'token is being transfered to valid address');
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) public {
        require(_exists(tokenId), 'need the token exists');
...
contract CustomERC721Token is ERC721Metadata {
    ...
    constructor (string memory name, string memory symbol) public ERC721Metadata(name, symbol, BASE_TOKEN_URI) {
